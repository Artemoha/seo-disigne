	...
	BX.Sale.BasketComponent = {
		...
		checkTotalAnimation: function(totalData)
		{
			if (this.result && this.result.TOTAL_RENDER_DATA && parseFloat(this.result.TOTAL_RENDER_DATA.PRICE) > parseFloat(totalData.PRICE))
			{
				...
				totalData.PRICE_FORMATED = this.result.TOTAL_RENDER_DATA.PRICE_FORMATED.replace("руб.","₽");
		...
		checkBasketItemsAnimation: function(itemData)
		{
			...
			if (this.items[itemId])
			{
				...
				)
				{
					...
					itemData.SUM_PRICE_FORMATED = this.items[itemId].SUM_PRICE_FORMATED.replace("руб."," ₽");
		...
		createBasketItem: function(itemId)
		{
            ...
            if(this.items[itemId]['IBLOCK_ID'] === "41") {
                basketItemTemplate = this.getTemplate('basket-item-commission-template');
            } else {
                basketItemTemplate = this.getTemplate('basket-item-template');
            }
